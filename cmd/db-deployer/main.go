...
func main() {
	...
	// Get & Create PVC
	pvcString, ok := cm.Data[l2cv1.KeyDbPvc]
	if !ok {
		log.Fatalf("key %s does not exist in ConfigMap %s\n", l2cv1.KeyDbPvc, configMapName)
	}
	...
	default:
		log.Fatalf("%s should contain PersistentVolumeClaim yaml (currently %s)\n", l2cv1.KeyDbPvc, reflect.TypeOf(obj).String())
	}

	// Get & Create Service
	svcString, ok := cm.Data[l2cv1.KeyDbSvc]
	if !ok {
		log.Fatalf("key %s does not exist in ConfigMap %s\n", l2cv1.KeyDbSvc, configMapName)
	}
	...
	default:
		log.Fatalf("%s should contain Service yaml (currently %s)\n", l2cv1.KeyDbSvc, reflect.TypeOf(obj).String())
	}

	// Get & Create Secret
	secretString, ok := cm.Data[l2cv1.KeyDbSecret]
	if !ok {
		log.Fatalf("key %s does not exist in ConfigMap %s\n", l2cv1.KeyDbSecret, configMapName)
	}
	...
	default:
		log.Fatalf("%s should contain Secret yaml (currently %s)\n", l2cv1.KeyDbSecret, reflect.TypeOf(obj).String())
	}

	// Get & Create Deployment
	deployString, ok := cm.Data[l2cv1.KeyDbDeploy]
	if !ok {
		log.Fatalf("key %s does not exist in ConfigMap %s\n", l2cv1.KeyDbDeploy, configMapName)
	}
